...
line) with hierarchical buttons — Customer as Account Name → Type → Portal Group → Account,
Product as Division → Category → Range → Product — where clicking a bar drills in. Chart.js
is embedded, so the file opens on anyone's machine with no install and no internet;
the data it charts is embedded too, so sending the page means sending the numbers.
